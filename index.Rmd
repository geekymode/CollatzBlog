...
::: {.fullwidth}
While wandering through Warsaw a couple of weeks ago during a conference, I found myself on a familiar street that I'd walked many times before—the one that passes by the University of Warsaw library. The library's exterior walls are adorned with stunning inscriptions spanning humanity's intellectual heritage: ancient Sanskrit, Hebrew, Greek, Arabic, and Latin texts alongside musical notation and iconic scientific achievements like DNA structures and Maxwell's equations.
:::

```{r figCollatzWarsaw, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "The Warsaw University Library wall featuring aspects of music,culture and sciences.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
```{r figCollatz1, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "BLA", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
More than a decade ago, when I first encountered this wall, I was thrilled to spot Maxwell's equations among the carefully curated collection of human knowledge. But this time, something new caught my eye. Nestled above the Maxwell equations and DNA imagery was a small algorithmic snippet that I'd somehow missed before—a brief program written in Pascal.
...
The rules are almost childishly simple. Take any positive integer. If it's even, divide it by two. If it's odd, multiply by three and add one. Repeat this process with each resulting number. The Collatz conjecture^[Let $\mathbb{N} = \{0,1,2,\ldots\}$ denote the set of natural numbers and $\mathbb{N}^+ = \{1,2,\ldots\}$ denote the positive integers. The Collatz process involves repeatedly applying the function $C(x) = x/2$ for even $x \in \mathbb{N}$ and $C(x) = 3x + 1$ for odd $x$. Since $3x + 1$ yields an even result whenever $x$ is odd, we can define a streamlined variant $T$ where $T(x) = x/2$ for even $x$ and $T(x) = (3x + 1)/2$ for odd $x$. The Collatz conjecture asserts that repeated application of either $T$ or $C$ to any positive integer $x \in \mathbb{N}^+$ eventually reaches 1. Throughout this work, we employ $T$ exclusively due to its computational advantages^4^. As an example, applying $T$ iteratively starting from $x = 45$ produces:] claims that no matter which positive integer you start with, you will always eventually reach the number 1.

Consider starting with 7, an arbitrary odd number. Following our rules: 7 becomes 22 (3×7+1), then 11, then 34, then 17, then 52, then 26, then 13, then 40, then 20, then 10, then 5, then 16, then 8, then 4, then 2, and finally 1. Once you reach 1, the sequence enters an endless loop: 1, 4, 2, 1, 4, 2, and so on.

This behavior has been verified for every starting number tested, including all integers up to approximately 2^68—that's roughly 295 quintillion numbers, Barina<sup>[1](#ref1)</sup>. Computers have crunched through these calculations for decades, yet not a single counterexample has emerged. Despite this overwhelming computational evidence, no one has managed to prove that the pattern holds universally.
...
Alternatively, for the streamlined variant $T$:
...
The Collatz sequence starting from $x = 21$ using the function $f$ is:
...
```{r figAll, fig.margin = TRUE, fig.cap = "TBD.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
```{r figCollatz20Kmath, fig.margin = TRUE, fig.width= 8, fig.height= 8, fig.cap = "The first 10000 collatz sequences.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/collatz_math_10k.png")
```
...
```{r figCollatzTree3DFull, fig.margin = FALSE, fig.cap = "3D view of the collatz sequence.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
The sequence reaches 1 in 6 steps.
```
...
```{r figCollatz10K, fig.margin = FALSE, fig.fullwidth = TRUE, fig.width= 8, fig.height= 8, fig.cap = "The first 10000 collatz sequences depicted like an underwater plant swaying under the influence of the ocean current.", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/fig_collatz10K.png")
```

 The longer, more complex sequences resembling the flowing tendrils of giant kelp that drift and undulate before joining the deeper currents, while shorter sequences mirror the direct, upward growth of simpler marine plants. This aquatic metaphor captures the organic, almost living quality of the Collatz structure, where mathematical sequences flow together in patterns that echo the natural convergent behavior found in ocean ecosystems, creating a unified, tree-like organism rooted in the bedrock of 1.
...
```{r figFractal1, fig.margin = FALSE,fig.fullwidth = FALSE, fig.cap = "Collatz fractal. This is a familiar image that is widely shared over internet such as [https://njohnston.ca/2009/06/the-collatz-conjecture-as-a-fractal/] ", message=TRUE, cache=TRUE, echo=FALSE, out.width='100%'}
...
Zooming in reveals remarkably detailed, somewhat mystical structure emerges at the integer starting points.  
...
The standard approach to complexifying the Collatz function replaces the parity test with a continuous analogue. Since determining whether a complex number is "even" or "odd" isn't straightforward, mathematicians typically use the function:
$$f(z) = \frac{1}{4}  \left( 2 + 7z - (2 + 5z)\cos{\pi z} \right)$$
This formulation smoothly interpolates between the two Collatz operations: when z is an even integer, $\cos(\pi z) = 1$, giving $f(z) = z/2$. When z is an odd integer, the reverse occurs, yielding $f(z) = 3z + 1$.
...
The fractal structure of the complex Collatz function provides geometric insights into the original conjecture. Regions of convergence in the complex plane can illuminate why certain integer sequences behave as they do. The fractal boundaries might encode information about the distribution of Collatz trajectory lengths or reveal hidden patterns in the conjecture's behavior.

Color-coding points by convergence rate, trajectory length, or limiting behavior produces images of extraordinary complexity and aesthetic appeal. 

## Mathematical Significance
The study of complex Collatz fractals represents more than mathematical artistry. The complex Collatz fractals ultimately illustrate how a simple discrete rule, when extended thoughtfully to the complex plane, can generate infinite complexity and beauty while potentially offering new perspectives on one of mathematics' most enduring mysteries.
